...
[![Abdeckung Domäne](https://img.shields.io/endpoint?url=https%3A%2F%2Faco993.github.io%2FWorkPlanStudio%2Fbadges%2FWorkPlanStudio.Domain.json)](docs/TESTING.de.md#abdeckung)
...
- **Abdeckung je Assembly in der CI abgesichert, und die Schwelle selbst ebenso** — der Build scheitert, wenn eine Assembly unter ihre Schwelle fällt, und ebenso, wenn die Abdeckung **mehr als drei Punkte darüber** steigt: eine Schwelle, der die Abdeckung davongelaufen ist, verteidigt nichts mehr und wird nie rot dabei. Die Schwellen stehen an genau einer Stelle, in den `GATE_*`-Variablen von [`ci.yml`](.github/workflows/ci.yml), und liegen ein bis zwei Punkte unter dem Gemessenen. **Was gemessen wurde, steht auf den Badges oben**, erzeugt bei jeder Auslieferung aus demselben Lauf — sie *sind* die Zahl und nicht eine Kopie davon. Dieser Punkt trug die sechs Werte früher von Hand; als die Drift-Prüfung entstand, waren zwei davon drei Punkte hinter der Abdeckung zurück, die sie bewachen sollten.
