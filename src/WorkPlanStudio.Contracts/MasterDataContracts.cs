...
/// <summary>
/// An accounting cost center — the unit a work center's machine hours are booked
/// against.
/// <para>
/// It is its own resource because it is its own master data: several work centers
/// book against one cost center, and as free text the same center arrived as
/// <c>CC-2000</c>, <c>cc-2000</c> and <c>CC 2000</c>, which made the only question
/// worth asking of it — what does CC-2000 cost per hour in total — unanswerable.
/// See ADR 0016.
/// </para>
/// </summary>
/// <param name="Id">Server-assigned key. Zero on a create request.</param>
/// <param name="Code">The controlling key, unique across the tenant (case-insensitive).</param>
/// <param name="Name">What the cost center is called.</param>
/// <param name="Description">Optional free-text note; nothing computes from it.</param>
/// <param name="IsActive">Retired centers stay readable because work centers still point at them.</param>
...
/// The cost center this work center books against, or <c>null</c> when Controlling
/// has not assigned one. This is the only half of the relationship a write is read
/// from: a caller selects an existing cost center, it never invents one, and an id
/// no cost center owns is refused with <c>Val_CostCenterMissing</c> rather than
/// created on the way past.
/// </param>
/// <param name="CostCenterCode">That cost center's code; ignored on write.</param>
/// <param name="CostCenterName">That cost center's name; ignored on write.</param>
...
/// <param name="ReleaseLocal">
/// Earliest moment work may start, as <b>plant-local wall-clock time</b> with no
/// offset. It was called <c>ReleaseUtc</c> and never was UTC; the shift calendar,
/// the Sunday rest window and the holiday closures all read it as the clock on the
/// shop floor, so the name was the only thing wrong with it. A caller must send it
/// unconverted — applying an offset here moves work by whatever the caller's own
/// zone happened to be.
/// </param>
/// <param name="DueLocal">When the customer expects it, in the same plant-local wall clock.</param>
...
/// <param name="CreatedUtc">When the order was created. Genuine UTC, unlike the two planning dates.</param>
/// <param name="ModifiedUtc">When it last changed. Genuine UTC, unlike the two planning dates.</param>
/// <param name="ConcurrencyStamp">Opaque version token; see <see cref="WorkCenterDto.ConcurrencyStamp"/>.</param>
