...
public static class WorkCenterEndpoints
{
    ...
    public static void MapWorkCenterEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/", async (WorkCenterDto request, ApiDbContext db, CancellationToken cancellationToken) =>
        {
            ...
            return Results.Created($"/api/work-centers/{center.Id}", center.ToDto(ApiDbContext.StampOf(entry)));
        })
        ...
        group.MapPut("/{id:int}", async (int id, WorkCenterDto request, ApiDbContext db, CancellationToken cancellationToken) =>
        {
            ...
            // Deactivating a center that released routings still point at would
            // make those routings unschedulable without anybody being told.
            if (wasActive && !center.IsActive && await db.Operations.AnyAsync(
            ...
            return Results.Ok(center.ToDto(ApiDbContext.StampOf(entry)));
        })
        .RequireAuthorization(PolicyNames.ManageMasterData)
        ...
        .RequireAuthorization(PolicyNames.ManageMasterData)
        .WithSummary("Deletes a work center no operation references.");
