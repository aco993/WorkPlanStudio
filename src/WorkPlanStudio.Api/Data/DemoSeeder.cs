...
public static class DemoSeeder
{
    ...
    public static async Task<bool> EnsureSampleDataAsync(ApiDbContext db, CancellationToken cancellationToken = default)
    {
        ...
        // Cost centres first: they are master data in their own right, and the
        // codes and names are the browser app's, so a planner who pulls this
        ...
        var inspect = new WorkCenter { Code = "QC-900", Name = "Quality Inspection", CostCenter = quality, HourlyRate = 55m, ShiftPatternKey = ShiftPatterns.OneShift.Key };
        ...
        //
        // Monday 1 June 2026, 06:00 on the plant's own clock — the week of
        // Fronleichnam, a holiday in the default state, so the calendar visibly
        // shapes the plan. Unspecified on purpose: these are wall-clock readings,
        // not instants, and a "Z" here would be a claim about a time zone the
        // model does not have.
        var horizon = new DateTime(2026, 6, 1, 6, 0, 0, DateTimeKind.Unspecified);

        // A genuine UTC stamp, unlike the horizon beside it.
        var recorded = new DateTime(2026, 5, 29, 9, 0, 0, DateTimeKind.Utc);

        db.ProductionOrders.AddRange(
            NewOrder("PO-1001", shaft, quantity: 60, horizon, horizon.AddDays(6), priority: 3, recorded),
            NewOrder("PO-1002", bracket, quantity: 120, horizon.AddHours(4), horizon.AddDays(5), priority: 1, recorded),
            NewOrder("PO-1003", shaft, quantity: 25, horizon.AddDays(1), horizon.AddDays(4), priority: 5, recorded));
    ...
    private static ProductionOrder NewOrder(
        string number, WorkPlan plan, int quantity, DateTime release, DateTime due, int priority, DateTime recordedUtc)
    {
        ...
        return new ProductionOrder
        {
            ...
            // The blob's index, so the database refuses to delete a work centre
            // one of these orders still needs — the same rows the release
            // endpoint writes.
            RoutingCenters = [.. snapshot.WorkCenterIds.Select(id => new OrderRoutingCenter { WorkCenterId = id })],
            CreatedUtc = recordedUtc,
            ModifiedUtc = recordedUtc
        };
    }
}
