...
public static class WorkCenterValidator
{
    ...
    /// <summary>Decimal places a money value may carry, now that the column can hold what it says.</summary>
    ...
    public static IReadOnlyList<ValidationIssue> Validate(WorkCenter center)
    {
        ...
        else if (Text.ExceedsScale(center.HourlyRate, MoneyScale))
            issues.Add(new(nameof(center.HourlyRate), "Val_Scale", MoneyScale));
    ...
    public static IReadOnlyList<ValidationIssue> ValidateAbsence(WorkCenterAbsence absence)
    {
        ...
        else if (Text.HasControlCharacters(absence.Label))
            issues.Add(new(nameof(absence.Label), "Val_SingleLine"));
