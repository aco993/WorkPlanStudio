...
- **Coverage gated per assembly in CI, and the gate is gated too** — the build fails when an assembly falls below its threshold, and also when coverage climbs **more than three points above** it, because a threshold coverage has outgrown defends nothing and stops being a gate without ever going red. The thresholds live in one place, the `GATE_*` variables in [`ci.yml`](.github/workflows/ci.yml), and each sits one to two points under what is measured. **What is measured is on the badges above**, written by the same CI run on every deployment — they are the number, rather than a copy of it that drifts. This bullet used to carry the six thresholds by hand; by the time the drift check was written, two of them had fallen three points behind the coverage they were guarding.
