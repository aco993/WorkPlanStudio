...
- **Coverage gated per assembly in CI** — the build fails below 95 % lines for the engine, 93 % for the working-time library, 97 % for the export writers, 87 % for the shared domain, 78 % for the app and 68 % for the backend. Each threshold sits about two points under what is measured, so it defends what has been reached rather than a number from the day it was written. **What is measured is on the badges above**, which are written by the same CI run on every deployment — they are the number, rather than a copy of it that drifts. This sentence used to carry the six values by hand; two of them had already fallen behind the badge beside them.
